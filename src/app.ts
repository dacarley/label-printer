...
import type { RawFormState, Orientation } from "./state/types";
import { loadSettings, normalizeSettings, saveSettingsDebounced } from "./state/settings";
import { setCSSVars, applyAlignment, setRotation, setPreviewText } from "./render/preview";
...
export function startApp() {
  ...
  const initial: RawFormState = {
    ...defaults,
    ...saved,
    // tolerate older keys if any
    orientation: (saved?.orient === "landscape" ? "landscape" : saved?.orient === "portrait" ? "portrait" : (saved?.orientation as any)) ?? defaults.orientation,
    text: (saved?.txt as any) ?? (saved?.text as any) ?? defaults.text,
    w: (saved?.w as any) ?? defaults.w,
    h: (saved?.h as any) ?? defaults.h,
    pad: (saved?.pad as any) ?? defaults.pad,
    align: (saved?.align as any) ?? defaults.align,
  };
  ...
  let orientation: Orientation = initial.orientation === "landscape" ? "landscape" : "portrait";
  ...
  function applyOrientationButtonState() {
    els.portraitBtn.setAttribute("aria-pressed", orientation === "portrait" ? "true" : "false");
    els.landscapeBtn.setAttribute("aria-pressed", orientation === "landscape" ? "true" : "false");
  }
  ...
  function updateAll() {
    ...
    if (!fit.minFits) show(els.overflowWarn); else hide(els.overflowWarn);

    // Keep print DOM warm
    try { renderPrintRoot(els.printRoot, s); } catch {}
  ...
  function blurAll() {
    ...
    try { els.txt.blur(); } catch {}
    try { els.w.blur(); els.h.blur(); els.pad.blur(); } catch {}
  }
  ...
  els.okModalBtn.addEventListener("click", () => { closeModal(els.modal); save(); });
  ...
  window.addEventListener("orientationchange", onResize as any, { passive: true } as any);

  // ESC closes modal
  window.addEventListener("keydown", (e) => {
    if (e.key === "Escape" && els.modal.classList.contains("open")) closeModal(els.modal);
  });
